...
| `github.request_timeout_ms` | the wall clock of one `gh` invocation. What it may hand back is bounded separately, by the code: 32 MiB per response, the workpad is looked for in an issue's first 1,000 comments, and a board poll reads at most 1,000 open issues under one state label (5,000 closed ones in the sweep that finishes them). A board past that is a failed read, which holds dispatch and says so, rather than a short one the worker would claim from | `30000` |
