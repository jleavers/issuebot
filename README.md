...
on the command line are loaded, and issuebot names none. Nothing in `/home/agent/.claude.json`,
and no `.mcp.json` in a repository issuebot clones, reaches a session — including anything an
earlier session wrote there. Adding an MCP server for the agent is therefore not a matter of
`claude mcp add` inside the container; it would need a change to the argv issuebot builds.
