...
| `claude.setting_sources` | which Claude Code settings sources the agent loads (`user`, `project`, `local`); `project` or `local` makes the clone's `CLAUDE.md` and `.claude/` its configuration, which `validate` warns about (`.mcp.json` stays out under `--strict-mcp-config` either way, and what those files `@`-include stays inside the workspace under `--settings claudeMdExcludes`, whatever a previous session approved in `~/.claude.json`) | `[user]` |
