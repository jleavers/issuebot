...
4. When the PR is open and its checks are green, the agent labels the issue
   `issuebot/review`. Checks that Actions never ran do not hold a finished issue — see
   [Checks that never ran](docs/operations.md#checks-that-never-ran) for how to tell those
   from a job that ran and failed, which still does. If Slack is configured, that state
   change is posted. An issue whose reported behaviour no longer happens reaches the same
   label by the other route: the agent records the reproduction it ran and what it found
   instead in the workpad, and hands over with no PR attached.
