...
session. Both builds must also report `LANG=C.UTF-8` under `sh -c` and under `bash -lc`, and
a bare `initdb` in the opt-in one must land on `UTF8` (#66): the base image sets no locale,
on `C` a cluster comes out `SQL_ASCII`, and pinning the encoding catches that rather than
the variable that happens to produce it.
