...
def _remove_path(path: Path, what: str) -> None:
    ...
    Except one: a path that is *already gone* is the state this was asked to reach, so it is
    ...
    so a broken symlink left at the name is a remnant and not a success.
    """
    try:
        if path.is_dir():
            shutil.rmtree(path)
