...
`load_workflow(path)` → `Workflow(config: Settings, prompt_template,
...
`configure_logging()` (structlog, JSON to stderr by default),
`get_logger()`, `bind_issue_context()`, `bind_session_context()`, `clear_context()`.

## `issuebot.pipes`

`read_capped(stream, limit, on_overrun)`, a leaf module for the same reason
...
`run_hint(subcommand)`, the clause a message names to say how to run
...
the shape of `database.url`, a leaf module because `issuebot.db` imports
...
the allow-listing `CONNECT` proxy that bounds the session's network
...
frozen dataclass events (`EVENT_KINDS`), `EventBus.publish()`
(synchronous, sink failures isolated and counted), `LogSink`. `RunEnded.log_dir` (Phase 6)
carries the run's log directory.

## `issuebot.github`

`StateLabel` roles, the transition table, `model_label_style` and
...
"a session does not leave *configuration* in a credential file", and what survives is the
credential state: `oauth_token` and `user`, neither of which `-h` can write. The keys come out at *both* levels
...
one asyncio task owns the schedule. `admission.py` (pure, #112) is
