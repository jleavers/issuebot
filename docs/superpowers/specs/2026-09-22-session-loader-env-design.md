...
Then the channel end to end through issuebot's own seam, unprotected:

```text
$ uv run python .issuebot/probe/repro.py
workspace_env_applied  keys=['LD_PRELOAD', 'LD_AUDIT', 'LD_LIBRARY_PATH', 'DATABASE_URL']
reaches the environment: {'LD_PRELOAD': '...libmemusage.so', 'LD_AUDIT': '...', 'LD_LIBRARY_PATH': '...'}
```
...
**All four are protected: `LD_PRELOAD`, `LD_AUDIT`, `LD_LIBRARY_PATH` — and
`LD_TRACE_LOADED_OBJECTS`, which the measurements turned up and which is the worst of them.**
...
- **A `RUNPATH` baked in at link time** — `-Wl,-rpath`, or `LD_RUN_PATH`, which is binutils
  ...
  rather than a workaround: `ld.so(8)` describes `LD_LIBRARY_PATH` as a facility for testing and
  debugging, and a built artefact that needs a library at run time has `RUNPATH` for it. It is
...
- **The rest of `ld.so(8)`'s environment, out by the rule and measured rather than assumed.**
  `LD_BIND_NOW` and `LD_DYNAMIC_WEAK` change how symbols bind; `LD_DEBUG`, `LD_DEBUG_OUTPUT` and
  ...
  loaded. `GLIBC_TUNABLES` is glibc's tunables namespace — allocator and hwcap parameters, no
  object — and is named here because it is the other name a reader will ask about.
...
`tests/test_agent_runner.py`: `merge_workspace_env` refuses each of the four, one parametrised
case per name annotated with what the loader does with it, and still applies an ordinary key
beside it. Eight negative cases pin that the bound stops where it says it does — `LD_RUN_PATH`
first among them, with `LD_BIND_NOW`, `LD_DYNAMIC_WEAK`, `LD_DEBUG`, `LD_DEBUG_OUTPUT`,
`LD_PROFILE`, `GLIBC_TUNABLES` and `LDFLAGS`. `LOADER_ENV_NAMES` is pinned as a list, as the
...
`workspace_environment` with a real `.issuebot/env`, a file carrying all four plus `LD_RUN_PATH`
and a `DATABASE_URL` hands the next turn its DSN *and its `LD_RUN_PATH`* and nothing else, with
one `workspace_env_ignored` line per refused key naming the key and never its value.

`tests/test_agent_workspace.py`: the channel through the shell a hook actually gets, and
compiler-free — two of the four are visible without building anything. A `before_run` hook writes
all four plus a `DSN` into `.issuebot/env`, with `LD_PRELOAD` naming a file that is not an ELF
object; the `after_run` hook — run through `bash -lc`, the shipped `hook_shell` — prints
...
`PROTECTED_ENV_NAMES`, all seven of these tests fail, and the workspace one fails by printing
the *shell's own library list* where `hook-ran` should be, with the loader's `LD_PRELOAD` and
`LD_AUDIT` complaints about the planted file on its stderr — and `exit_code: 0`.

```text
E       AssertionError: assert ['\tlinux-vds...20db51f7000)'] == ['hook-ran', ...issuebot@/db']
E         At index 0 diff: '\tlinux-vdso.so.1 (0x00007ffd055fb000)' != 'hook-ran'
