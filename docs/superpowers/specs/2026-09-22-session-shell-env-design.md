...
- **The dynamic loader: `LD_PRELOAD`, `LD_AUDIT`, `LD_LIBRARY_PATH`.** *Closed by #187
  ...
  `LD_TRACE_LOADED_OBJECTS` beside them, in `LOADER_ENV_NAMES`. The reasoning below for leaving
  them out is kept as written, because #187 overturns part of it: the "different question with a
  different answer" turned out to have the same answer, `LD_LIBRARY_PATH` included, once a file
  planted at a needed soname was measured loading into `git` with no `LD_PRELOAD` anywhere.*
