...
- **The session's authority** — below, as
  [one account per concurrent session](#one-account-per-concurrent-session). `claude -p`, every
  hook and the clone run as a session account that is not the worker's, cannot invoke `sudo`,
  and cannot read the worker's home, its code or the rest of its environment. A way for a
  session to reach any of those, or to become the worker or another session's account, is in
  scope.
- **The session's network** — below, as
  [what a session may reach](#what-a-session-may-reach). Under Compose the worker's container
  has no route off the host except an allow-listing `CONNECT` proxy. A way to reach a host that
  is not on the allow list, or to send `GH_TOKEN` somewhere it should not go, is in scope.
...
The rest of this file is the detail behind those first three: the three things that fix what
one unattended session may do, none of which is the prompt. Each is reported by a line of
`issuebot validate`, and each is set outside anything an issue or the agent can write.

## How a session is bounded

### What a session may reach

`validate` reports this as its `egress` line. It is the third bound on what a session can do,
beside the token it holds and the tools it holds it with. Under Compose the worker's networks are all `internal`,
