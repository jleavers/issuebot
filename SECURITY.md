...
- **The session's authority** — in detail as [one account per concurrent
  session](docs/security-model.md#one-account-per-concurrent-session). `claude -p`, every hook
  and the clone run as a session account that is not the worker's, cannot invoke `sudo`, and
  cannot read the worker's home, its code or the rest of its environment. A way for a session
  to reach any of those, or to become the worker or another session's account, is in scope.
- **The session's network** — in detail as [what a session may
  reach](docs/security-model.md#what-a-session-may-reach). Under Compose the worker's container
...
[**How a session is bounded**](docs/security-model.md) is the detail behind the first three of
those: the three things that fix what one unattended session may do, none of which is the
prompt. Each is reported by a line of `issuebot validate`, and each is set outside anything an
issue or the agent can write.
